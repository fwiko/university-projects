...
import controller.managers.MessageManager;
import controller.messages.MessageInbound;
import controller.messages.MessageOutbound;
import controller.messages.types.MessageOutboundType;
...
public class ControllerClient {
    ...
    public ControllerClient(InetAddress ipAddress, int portNumber, InetAddress loadBalancerIpAddress, int loadBalancerPortNumber) {
    ...
    public void start() {
        ...
        // Attempt to start the Message Manager, listening on the Controller socket
        try {
            messageManager.start(ipAddress, portNumber);
        } catch (IOException exception) {
            System.err.printf("ControllerClient - ERROR: IOException when opening Datagram Channel on %s:%d\n", ipAddress.getHostAddress(), portNumber);
            System.exit(-1);
        }
            ...
            public void run() {
                while (!Thread.currentThread().isInterrupted() && !messageManager.isStopped()) {
                    ...
                    try {
                        ...
                        System.err.printf("ControllerClient - ERROR: Could not handle %s Message (%s)", message.getType(), exception.getMessage());
                    }
                }
            }
        };
        ...
        System.out.print("### STARTED CONTROLLER CLIENT ###");
        ...
        while (!messageManager.isStopped()) {
            ...
            while (commandString.length() < 1) {
                ...
                try {
                    ...
                    System.err.printf("ControllerClient - ERROR: Failed to get User-input (%s)\n", exception.getMessage());
                }
            }
            ...
            try {
                ...
                System.err.printf("ControllerClient - ERROR: Failed to handle \"%s\" (%s)", commandString, exception.getMessage());
            }
        }
        
        System.out.println("ControllerClient - INFO: Stopped...");
    }
    ...
    private void handleCommand(String commandString) throws IllegalArgumentException {
        ...
        switch(args[0].toUpperCase()) {
            case "JOB" -> {
                ...
                // Check if registered
                if (!registered) {
                    System.out.println("ControllerClient - INFO: Unable to create a new Job (not registered)");
                }
            ...
            case "STOP" -> {
                ...
                // Check if registered
                if (!registered) {
                    System.out.println("ControllerClient - INFO: Unable to stop the Load-Balancer (not registered)");
                }
    ...
    private void handleMessage(MessageInbound message) throws IllegalArgumentException {
        switch (message.getType()) {
            case NEW_JOB_SUCCESS -> {
                ...
                System.out.printf("\rControllerClient - INFO: The Load-Balancer has successfully queued Job %s\n", message.getArgument(0));
            ...
            case NEW_JOB_FAILURE -> {
                ...
                System.err.printf("\rControllerClient - ERROR: The Load-Balancer failed to queue Job %s\n", message.getArgument(0));
            ...
            case REG_SUCCESS -> {
                ...
                System.out.printf("\rControllerClient - INFO: Successfully registered with the Load-Balancer on socket %s:%d\n", loadBalancerIpAddress.getHostAddress(), loadBalancerPortNumber);
            ...
            case FIN_JOB -> {
                ...
                System.out.printf("\rControllerClient - INFO: The execution of Job %s has finished\n", message.getArgument(0));
            ...
            case INFO -> {
                ...
                System.out.printf("\rControllerClient - INFO: Received information from the Load-Balancer \n\n%s\n", informationString);
    ...
    private void showOptions() {
        System.out.println(
                ...
                STOP                    Shutdown the LoadBalnacer System, all Nodes, and the Controller
                """
        );
    }
    
    private void register() {
        // Create a new MessageOutbound object with the REG_CONTROLLER Type and send to the Load-Balancer socket
        messageManager.sendMessage(new MessageOutbound(MessageOutboundType.REG_CONTROLLER, ipAddress.getHostAddress(), String.valueOf(portNumber)), loadBalancerIpAddress, loadBalancerPortNumber);
    }
