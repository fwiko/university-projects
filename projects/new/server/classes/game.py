...
class Game:
    ...
    def add_client(self, client: client.Client) -> None:
        if client in self.settings.players:
            return
        self.settings.players.append(client)
        self.alert(f"{client.username} has joined the game!")
        self.logger.debug(f"Added client {client.username}")
    ...
    def alert(self, message: str) -> None:
        self._send_data("alert", {"message": message})
    ...
    def start_quiz(self) -> None:
        ...
        scores_table = {p.uid: 0 for p in self.settings.players}
    ...
    def _quiz_sequence(self) -> None:
        ...
        time.sleep(5)
        for q, a in self.__current_quiz.questions:
